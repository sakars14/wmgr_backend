...
INFLATION_RATE_ANNUAL_DEFAULT = 0.06
EXPECTED_RETURN_ANNUAL_BY_RISK_BAND = {
    "Conservative": {"equity": 0.10, "debt": 0.07},
    "Balanced": {"equity": 0.12, "debt": 0.07},
    "Aggressive": {"equity": 0.13, "debt": 0.07},
}
HORIZON_RETURN_MIX_RULES = {
    "debtMaxYears": 3,
    "blendedMaxYears": 7,
    "blendedMix": {"equity": 0.5, "debt": 0.5},
}


def _normalize_risk_label(label: Any) -> str:
    raw = (str(label) if label is not None else "").strip().lower()
...
def _round_to_base(value: float, base: int = 100) -> float:
    if base <= 0:
        return value
    return round(value / base) * base


def _assumed_return_for_horizon(risk_label: str, horizon_years: float) -> float:
    returns = EXPECTED_RETURN_ANNUAL_BY_RISK_BAND.get(risk_label) or EXPECTED_RETURN_ANNUAL_BY_RISK_BAND["Balanced"]
    equity_return = returns.get("equity", 0.0)
    debt_return = returns.get("debt", 0.0)
    if horizon_years <= HORIZON_RETURN_MIX_RULES["debtMaxYears"]:
        return debt_return
    if horizon_years < HORIZON_RETURN_MIX_RULES["blendedMaxYears"]:
        mix = HORIZON_RETURN_MIX_RULES["blendedMix"]
        return equity_return * mix["equity"] + debt_return * mix["debt"]
...
def build_financial_blueprint(profile: Dict[str, Any]) -> Dict[str, Any]:
    ...
    months_target_raw = emergency.get("monthsTarget")
    expense_raw = cash_flow.get("totalMonthlyExpense")

    months_target = _safe_float(months_target_raw)
    dedicated_amount = _safe_float(emergency.get("dedicatedAmount"))
    current_dedicated = dedicated_amount

    missing_fields: List[str] = []
    if _is_blank(months_target_raw):
        missing_fields.append("emergency.monthsTarget")
    if _is_blank(expense_raw):
        missing_fields.append("cashFlow.totalMonthlyExpense")

    if missing_fields:
        target_corpus = 0.0
        emergency_gap = 0.0
        emergency_surplus = 0.0
        emergency_status = "Unknown"
        warnings.append(
            "Emergency status unknown: missing " + ", ".join(missing_fields) + "."
        )
    else:
    ...
    risk_band_label = _normalize_risk_label(risk_quiz_label or risk_tolerance_self)
    risk_band_defaulted = _is_blank(risk_quiz_label) and _is_blank(risk_tolerance_self)

    allocatable_surplus = max(0.0, surplus["monthlySurplus"])
    goal_projections: List[Dict[str, Any]] = []
    for item in goal_items:
        notes: List[str] = []
        amount_today = _safe_float(item.get("amountToday"))
        horizon_years = _safe_float(item.get("horizonYears"))
        inflation_rate = INFLATION_RATE_ANNUAL_DEFAULT
        assumed_return = _assumed_return_for_horizon(risk_band_label, horizon_years)
        if risk_band_defaulted:
            notes.append("Risk band missing; used Balanced assumptions.")
        ...
        if amount_today > 0 and horizon_years > 0:
            future_cost = amount_today * ((1 + inflation_rate) ** horizon_years)
            n = int(round(horizon_years * 12))
            if n > 0:
                r = assumed_return / 12.0
                if abs(r) < 1e-9:
                    factor = float(n)
                else:
                    factor = ((1 + r) ** n - 1) / r
                if factor > 0:
                    required_raw = future_cost / factor
                    if required_raw <= allocatable_surplus:
                        status = "OnTrack"
                        shortfall_raw = 0.0
                    elif allocatable_surplus <= 0:
                        status = "NotFeasible"
                        shortfall_raw = required_raw
                    else:
                        status = "Shortfall"
                        shortfall_raw = required_raw - allocatable_surplus
                else:
                    notes.append("Invalid projection factor; check horizon/return assumptions.")
            else:
                notes.append("Missing or invalid goal horizon.")
        else:
            notes.append("Missing goal amount or horizon.")

        required_rounded = _round_to_base(required_raw, 100)
        shortfall_rounded = _round_to_base(shortfall_raw, 100)

        goal_projections.append(
            {
                "goalKey": item.get("key"),
                "label": item.get("label"),
                "priority": item.get("priority"),
                "horizonYears": item.get("horizonYears"),
                "amountToday": item.get("amountToday"),
                "inflationRateAnnual": inflation_rate,
                "futureCostInflated": future_cost,
                "assumedReturnAnnual": assumed_return,
                "requiredMonthlyInvestmentRaw": required_raw,
                "requiredMonthlyInvestmentRounded": required_rounded,
                "availableMonthlySurplus": allocatable_surplus,
                "status": status,
                "shortfallMonthlyRaw": shortfall_raw,
                "shortfallMonthlyRounded": shortfall_rounded,
                "notes": notes,
            }
        )
    ...
    out = {
        "meta": {
            ...
            "assumptions": {
                "inflationRateAnnualDefault": INFLATION_RATE_ANNUAL_DEFAULT,
                "expectedReturnAnnualByRiskBand": EXPECTED_RETURN_ANNUAL_BY_RISK_BAND,
                "horizonReturnMixRules": HORIZON_RETURN_MIX_RULES,
            },
        },
        "inputsSnapshot": {
            "personal": {
        ...
        "derived": {
            ...
            "goals": goal_items,
            "goalProjections": goal_projections,
            "risk": {
